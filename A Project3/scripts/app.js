const navToggleIcon = document.querySelector(".nav_toggle-icon");
...
const menuitem = document.querySelectorAll(".menu_item"); //42

const sections = document.querySelectorAll("main > section"); //33
...
if(window.localStorage.getItem("theme")==="dark-theme"){
  ...
  changebtn.innerHTML = lightThemeIcon
}

//obsever برای گرفتن سکشن ها
const obsever = new IntersectionObserver(observerHndler, { threshold: 0.5 });
function observerHndler(allSection) {
  allSection.map((sections) => {
    let sectionClassName = sections.target.className;
    let sectionmenuItem = document.querySelector(
      `.menu_item[data-section=${sectionClassName}]`
    );
    if (sections.isIntersecting) {
      sectionmenuItem.classList.add("menu_item--active");
    } else {
      sectionmenuItem.classList.remove("menu_item--active");
    }
  });
}
sections.forEach((sections) => {
  obsever.observe(sections);
});

// - Custom Functions -
function navigation(listItems, listItemactiveclass, contentItemshowclass) {
  listItems.forEach((listItem) => {
    listItem.addEventListener("click", function () {
      ...
      let content = this.getAttribute("data-content-id");
      document.querySelector(content).classList.add(contentItemshowclass);
    });
  });
}
...
navigation(Resumeitem, "Resume--active", "Resume-content--show");
navigation(portfolioitem, "portfolio-item--active", "portfolio-content--show");


// Event Listeners
navToggleIcon.addEventListener("click", function () {
  this.classList.toggle("nav_toggle-icon--open");
...
// loops روی خودش قرار بگیرید وفاصله را حفظ گند از بالا homeبرای کلیک روی ایتم ها که روی هر ایتم کلیک کردیم مثلا
menuitem.forEach((item) => {
  item.addEventListener("click", function (e) {
    e.preventDefault();
    removeActiveClass("menu_item--active");
